...
export const TableProvider = ({ children }) => {
  ...
  const getRestaurantBookings = () => {
    return axios.get(`${process.env.REACT_APP_API_URL}/api/business/booking`).then(res => {
      dispatch({
  ...
  const getTables = () => {
    return axios
      .get(process.env.REACT_APP_API_URL + '/api/business/table/')
      .then(res => {
        dispatch({
          type: 'GET_TABLES',
          tables: res.data
  ...
  const getAvailableTimeslots = (id, covers, date, month, year) => {
    return axios.get(`${process.env.REACT_APP_API_URL}/api/timeslots/${id}/${covers}/${date}/${month}/${year}`)
    .then(res => {
      dispatch({
        type: 'TIMESLOTS',
  ...
  const getUserBookings = () => {
    axios.get(process.env.REACT_APP_API_URL + '/api/user/bookings', {withCredentials: true})
      .then(res => {
        dispatch({
  ...
  const getRestaurants = () => {
    axios.get(process.env.REACT_APP_API_URL + '/api/restaurants').then(res => {
      dispatch({
  ...
  const getRestaurantInfo = id => {
    axios.get(`${process.env.REACT_APP_API_URL}/api/restaurant/${id}`).then(res => {
      dispatch({
  ...
  const updateUserProfile = data => {
    axios.put(process.env.REACT_APP_API_URL + '/api/user/profile/update', data, {withCredentials: true}).then(() => navigate('/user/profile'))
  };

  const updateRestaurantProfile = data => {
    axios.put(process.env.REACT_APP_API_URL + '/api/business/profile/update', data, {withCredentials: true}).then(() => navigate('/business/profile'))
  };
  ...
  const authBusinessLogin = data => {
    axios.post(
          process.env.REACT_APP_API_URL + '/api/business/auth/login',
          data,
          {withCredentials: true}
        )
        .then(resp => {
  ...
  const authLogout = () => {
    axios.delete(process.env.REACT_APP_API_URL + '/api/auth/logout', {withCredentials: true}).then(() => {
      dispatch({
        type: 'AUTH',
        login: false,
        authType: '',
